  ...
  return (
    <Container>
      ...
      {isConnected &&
        <Desc>
          <FlexRow>
            <BaseText>Main page</BaseText>
            <SmallTextGrey>This is the landing page when people visit web.{sld}.{config.tld} </SmallTextGrey>
            <InputBox value={pageId} placeholder={'ae42787a7d...'} onChange={({ target: { value } }) => { setPageId(value) }}/>
          </FlexRow>
          <BaseText>Additional pages</BaseText>
          ...
          {allowedPageIds.map((pid, i) => {
            return <FlexRow key={pid}>
              <InputBox value={pid} onChange={({ target: { value } }) => { setAllowedPageIds(e => [...e.slice(0, i), value, ...e.slice(i + 1)]) }}/>
              <Button disabled={pending} $width={'auto'} onClick={ () => { setAllowedPageIds(e => [...e.slice(0, i), ...e.slice(i + 1)]) }}>
          ...
          <Button disabled={pending} $width={'auto'} onClick={ save}>{pending ? <Loading/> : 'SAVE' }</Button>
        </Desc>
      }
